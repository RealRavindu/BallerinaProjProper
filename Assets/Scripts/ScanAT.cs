...
namespace NodeCanvas.Tasks.Actions {
	...
	public class ScanAT : ActionTask {
		...
		public float radius;
		public BBParameter<Transform> lightHouseTransform;
		float t;
		...
		protected override void OnExecute() {
			t = 0;
			radius = agent.GetComponent<Blackboard>().GetVariableValue<float>("scanRadius");
		}
		...
		protected override void OnUpdate() {
			...
			DrawCircle(agent.transform.position, radius, scanColour, numberOfScanCirclePoints);
			Collider[] objectsInRange =Physics.OverlapSphere(agent.transform.position, radius, targetMask);

			foreach (Collider collider in objectsInRange)
			{
				Blackboard lightHouseBlackboard = collider.GetComponentInParent<Blackboard>();
                if (lightHouseBlackboard == null)
                {
					Debug.Log("Couldn't access the lighthouse blackboard on a parent of a collider on lighthouse layer[" + collider.gameObject.name+"]");
					continue;
                }
                float repairValue = lightHouseBlackboard.GetVariableValue<float>("repairValue");
				lightHouseTransform.value = lightHouseBlackboard.GetVariableValue<Transform>("workpad");
